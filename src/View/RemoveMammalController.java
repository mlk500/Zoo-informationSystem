...
public class RemoveMammalController implements Initializable{

    @FXML
    private ComboBox<Mammal> mammalList;
    ...
    void removeMammal(ActionEvent event) {
    	 Mammal m=mammalList.getSelectionModel().getSelectedItem();
     	if(mammalList.getSelectionModel().isEmpty()) {
 			Alerts.alertBox(AlertType.ERROR,"Invalid","Invalid Input", "" );		
     	}
     	else if(!(Zoo.getInstance().removeMammal(m))) {
 			Alerts.alertBox(AlertType.ERROR,"Failed","The remove method has failed", "" );		

     	}
     	else {
        	Zoo.getInstance().removeMammal(mammalList.getSelectionModel().getSelectedItem());//we remove the Mammal from our hashmap
     	Alerts.alertBox(AlertType.CONFIRMATION,"Success","The Mammal has been removed succsessfully", "" );		
	...
	public void initialize(URL arg0, ResourceBundle arg1) {//this function occurs immediately when we open the gui window which means that the item that we removed from the hashmap in the lsat time will not appear here because it will be updated
    	mammalList.getItems().clear();//first we want to clear out combobox and then we add all the items to it
        //we add all of our items back to the list
    	mammalList.getItems().addAll(Zoo.getInstance().getMammals().values());
