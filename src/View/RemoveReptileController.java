...
public class RemoveReptileController implements Initializable {

    @FXML
    private ComboBox<Reptile> reptileList;
    ...
    void removeReptile(ActionEvent event) {
    	 Reptile r=reptileList.getSelectionModel().getSelectedItem();
      	if(reptileList.getSelectionModel().isEmpty()) {
  			Alerts.alertBox(AlertType.ERROR,"Invalid","Invalid Input", "" );		
      	}
      	else if(!(Zoo.getInstance().removeReptile(r))) {
  			Alerts.alertBox(AlertType.ERROR,"Failed","The remove method has failed", "" );		

      	}
      	else {
        	Zoo.getInstance().removeReptile(reptileList.getSelectionModel().getSelectedItem());//we remove the Reptile from our hashmap
      	Alerts.alertBox(AlertType.CONFIRMATION,"Success","The Reptile has been removed succsessfully", "" );		
	...
	public void initialize(URL arg0, ResourceBundle arg1) {//this function occurs immediately when we open the gui window which means that the item that we removed from the hashmap in the lsat time will not appear here because it will be updated
    	reptileList.getItems().clear();//first we want to clear out combo box and then we add all the items to it
        //we add all of our items back to the list
    	reptileList.getItems().addAll(Zoo.getInstance().getReptiles().values());
